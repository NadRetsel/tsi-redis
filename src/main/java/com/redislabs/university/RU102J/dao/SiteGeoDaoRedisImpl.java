...
public class SiteGeoDaoRedisImpl implements SiteGeoDao {
    ...
    public Set<Site> findAll() {
        ...
        try (Jedis jedis = jedisPool.getResource())
        {
            // Single round trip to server to get keys
            Set<String> keys = jedis.zrange(RedisSchema.getSiteGeoKey(), 0, -1);


            // Pipeline commands to get all sites from the keys to another single round trip
            Pipeline pipeline = jedis.pipelined();

            List<Response<Map<String, String>>> allResponses = keys.stream()
        ...
        /* Original implementation with potentially many round trips to the server
    ...
    private Set<Site> findSitesByGeoWithCapacity(GeoQuery query) {
        ...
        try (Jedis jedis = jedisPool.getResource()) {
            // START Challenge #5
            // TODO: Challenge #5: Get the sites matching the geo query, store them in List<GeoRadiusResponse> radiusResponses;
